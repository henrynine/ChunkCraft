...
let player_with_three_logs = {player with inv = {player.inv with sets = [(Items.log, 3)]}; coords = 0,1}
...
"test get_blocks" >::
  (fun _ -> assert_equal (State.get_blocks (List.nth (List.nth (State.get_chunks map) 0) 0)) (blocks1));
...
"test pickup" >:: (fun _ -> assert_equal (State.move_player map_with_item_below 's') map_post_pickup);
...
"test take_first_item" >::
  (fun _ -> assert_equal (Blocks.take_first_item first_block_with_2_items) ((first_block_with_log, Items.wood_sword, 1)));
